...
RSpec.describe 'CommentsCreation', type: :controller do
  ...
  context 'with incorrect data' do
    it 'raises an RecordInvalid error' do
      expect{
        CommentsCreation.new(['Лучший коммент в мире'], ['The best comment in the world'], nil).rating
      }.to raise_error(ActiveRecord::RecordInvalid)
    end

    it 'raises an NoMethodError error' do
      expect{
        CommentsCreation.new(['Лучший коммент в мире'], nil, article.id).rating
